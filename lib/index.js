...
var useThree = function (callbacks, context) {
    ...
    var savedCallbacks = react_1.useRef(callbacks);
    var renderScene = react_1.useCallback(function (dt) {
        ...
        if (ref.current) {
            ...
            manager.current.onProgress = function (item, loaded, total) {
                if (savedCallbacks.current.onLoadProgress) {
                    savedCallbacks.current.onLoadProgress(storage.current, item, loaded, total);
                }
            };
